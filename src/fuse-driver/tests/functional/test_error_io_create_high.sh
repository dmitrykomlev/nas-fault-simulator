#!/bin/bash
#=============================================================================
# test_error_io_create_high.sh - High I/O error test on create operations
# 100% probability of -EIO errors on create operations
#=============================================================================
...
test_error_logic() {
    # Test parameters - 100% probability of I/O errors on create
    local num_creates=20  # Test with 20 create operations
    ...
    echo "Running HIGH I/O ERROR test on CREATE operations..."
    ...
    for i in $(seq 1 ${num_creates}); do
        ...
        if [ ${exit_code} -eq 0 ]; then
            ...
            # Create failed (this is what we expect 100% of the time)
    ...
    # For 100% error rate, we expect ALL creates to fail
    if [ ${failed_creates} -eq ${total_creates} ]; then
        echo "SUCCESS: All create operations failed as expected"
        report_result "Create Error Probability" 0 "All ${total_creates} creates failed (100% error rate)"
    elif [ ${failed_creates} -gt $((total_creates * 80 / 100)) ]; then
        echo "PARTIAL SUCCESS: Most creates failed (${actual_error_probability} error rate)"
        report_result "Create Error Probability" 0 "High error rate ${actual_error_probability} achieved"
    else
        echo "ERROR: Too many successful creates for 100% error config"
        report_result "Create Error Probability" 1 "Only ${actual_error_probability} error rate (expected 1.0)"
        return 1
    fi
...
run_test_with_config "error_io_create_high.conf" "error_io_create_high" test_error_logic